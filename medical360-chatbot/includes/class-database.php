...
class M360_Database {
    ...
    private const TABLE_CONVERSATIONS = 'medical360_conversations';
    private const TABLE_MESSAGES      = 'medical360_messages';
    private const TABLE_CONTENT_INDEX = 'medical360_content_index';
    private const TABLE_SETTINGS      = 'medical360_settings';
    ...
    public static function deactivate(): void {
        wp_clear_scheduled_hook( 'm360_cleanup_old_conversations' );
        wp_clear_scheduled_hook( 'm360_reindex_content' );
        wp_clear_scheduled_hook( 'm360_daily_leads_report' );
        wp_clear_scheduled_hook( 'm360_weekly_leads_report' );
    }
    ...
    public function seed_default_settings(): void {
        $defaults = [
            ...
            'greeting_he'       => 'שלום 👋 אני העוזר החכם של מדיקל קר. אשמח לעזור במידע על מחלקות, קבלה, מיקום וסדנאות.',
            'greeting_en'       => 'Hi 👋 I\'m the Medical Care smart assistant. Happy to help with departments, admission, location and workshops.',
            'greeting_ru'       => 'Здравствуйте 👋 Я умный помощник Medical Care. Помогу с отделениями, приёмом, расположением и мастер-классами.',
            'phone'             => '',
            'map_url'           => '',
            'widget_title_he'   => 'מדיקל קר — עוזר חכם',
            'widget_title_en'   => 'Medical Care — Smart Assistant',
            'widget_title_ru'   => 'Medical Care — Умный помощник',
            'max_context_docs'  => 5,
            ...
            'desktop_autoopen_msg_he'  => 'היי 👋 בואו לדבר עם העוזר החכם של מדיקל — אני כאן לכל שאלה!',
            'desktop_autoopen_msg_en'  => 'Hi 👋 Come chat with Medical\'s smart assistant — I\'m here for any question!',
            'desktop_autoopen_msg_ru'  => 'Здравствуйте 👋 Поговорите с умным ассистентом Medical — я здесь для любого вопроса!',
            // Marketing consent (Opt-In) — legal requirement before saving a marketing lead
            'consent_required'  => '1',
            'consent_version'   => '1.0',
            'consent_text_he'   => 'אני מסכים/ה לקבל מעמותת אמפא ארגון מרפא אינטגרטיבי (ע״ר), מספר ארגון 580713014, פניות שיווקיות לרבות בשיחת טלפון, SMS ודוא״ל. ההסכמה בתוקף לשנה וניתן לבטלה בכל עת.',
            'consent_text_en'   => 'I agree to receive marketing communications from Ampa Integrative Care NPO (reg. 580713014), including by phone, SMS and email. This consent is valid for one year and can be withdrawn at any time.',
            'consent_text_ru'   => 'Я согласен(на) получать маркетинговые сообщения от Ampa (рег. 580713014), в том числе по телефону, SMS и эл. почте. Согласие действует один год и может быть отозвано в любое время.',
    ...
    private function schedule_cleanup(): void {
        if ( ! wp_next_scheduled( 'm360_cleanup_old_conversations' ) ) {
            wp_schedule_event( time(), 'daily', 'm360_cleanup_old_conversations' );
        }
        add_action( 'm360_cleanup_old_conversations', [ $this, 'cleanup_old_conversations' ] );
    }

    private function schedule_reindex(): void {
        if ( ! wp_next_scheduled( 'm360_reindex_content' ) ) {
            wp_schedule_event( time(), 'daily', 'm360_reindex_content' );
        }
        add_action( 'm360_reindex_content', function () {
            M360_Content_Indexer::get_instance()->full_reindex();
        } );
    }
    ...
    public function register_reports(): void {
        ...
        if ( ! wp_next_scheduled( 'm360_daily_leads_report' ) ) {
            wp_schedule_event( strtotime( 'tomorrow 8:00' ), 'daily', 'm360_daily_leads_report' );
        }
        if ( ! wp_next_scheduled( 'm360_weekly_leads_report' ) ) {
            wp_schedule_event( strtotime( 'next monday 8:00' ), 'weekly', 'm360_weekly_leads_report' );
        }
        add_action( 'm360_daily_leads_report',  fn() => $this->send_leads_report( 'day' ) );
        add_action( 'm360_weekly_leads_report', fn() => $this->send_leads_report( 'week' ) );
    }
    ...
    public function send_leads_report( string $period ): void {
        ...
        $leads  = array_reverse( (array) get_option( 'm360_leads', [] ) );
        ...
        $attachments = [];
        $up = wp_upload_dir();
        $file = trailingslashit( $up['basedir'] ) . 'medical360-leads-' . $period . '-' . date( 'Ymd-His' ) . '.csv';
    ...
    private function derive_key(): string {
        ...
        $secret = defined( 'AUTH_KEY' ) ? AUTH_KEY : 'medical360-fallback-secret';
        return substr( hash( 'sha256', 'm360:' . $secret, true ), 0, SODIUM_CRYPTO_SECRETBOX_KEYBYTES );
    }
