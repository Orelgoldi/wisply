/* Medical360 Chatbot v3.6.0 — Shadow DOM + real-time voice (OpenAI Realtime WebRTC, grounded) */
(function () {
  'use strict';

  const CFG = window.M360Config || {};
  ...
  /* ── Session ── */
  function newSid() {
    return 'm360-' + Date.now().toString(36) + Math.random().toString(36).slice(2);
  }
  let sid = (() => {
    let id = sessionStorage.getItem('m360_sid');
    if (!id) { id = newSid(); sessionStorage.setItem('m360_sid', id); }
    return id;
  })();
  ...
  function build() {
    ...
    shadow.innerHTML = `
      ...
      <div class="m-win" id="m-win" role="dialog" aria-modal="true" aria-label="Medical360 Chat" hidden>
  ...
  function botName()  { return S['widget_title_'+lang] || S.widget_title_he || 'מדיקל קר — עוזר חכם'; }
  function shortName(){ return botName().split(/[—–-]/)[0].trim() || 'מדיקלי'; }
  ...
  function captureContext() {
    if (!sessionStorage.getItem('m360_landing')) {
      sessionStorage.setItem('m360_landing', location.href);
      const p = new URLSearchParams(location.search);
      sessionStorage.setItem('m360_utm_source',   p.get('utm_source')   || '');
      sessionStorage.setItem('m360_utm_medium',   p.get('utm_medium')   || '');
      sessionStorage.setItem('m360_utm_campaign', p.get('utm_campaign') || '');
      sessionStorage.setItem('m360_referrer',     document.referrer     || '');
    }
  }
  function leadContext() {
    return {
      utm_source:   sessionStorage.getItem('m360_utm_source')   || '',
      utm_medium:   sessionStorage.getItem('m360_utm_medium')   || '',
      utm_campaign: sessionStorage.getItem('m360_utm_campaign') || '',
      referrer:     sessionStorage.getItem('m360_referrer')     || '',
      landing_page: sessionStorage.getItem('m360_landing')      || '',
  ...
  function fetchPageQuestions() {
    ...
    const key = 'm360_pq_' + lang + '_' + location.pathname;
  ...
  function setupAutoOpen() {
    if (sessionStorage.getItem('m360_autoopened')) return;   // don't nag on every page
    setTimeout(() => {
      if (isOpen || hadActivity) return;                     // respect the visitor
      sessionStorage.setItem('m360_autoopened', '1');
  ...
  function showSuggestions() {
    ...
    const chips   = usePage ? pageQuestions : t('chips');
  ...
  function toggleListen() {
    ...
    if (!speakMode) { speakMode = true; sessionStorage.setItem('m360_speak','1'); updateSpeakerUI(); }
    stopAudio();
  ...
  function toggleSpeak() {
    speakMode = !speakMode;
    sessionStorage.setItem('m360_speak', speakMode ? '1' : '0');
  ...
  /* ── Init ── */
  function init() {
    host = document.getElementById('m360-root') || document.getElementById('m360-chatbot-root');
    if (!host) return;
    host.id = 'm360-root';
