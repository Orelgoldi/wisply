...
if ( isset( $_GET['export'] ) && current_user_can( 'manage_options' ) ) {
    ...
    echo "\xEF\xBB\xBF";
    echo "שם,טלפון,אימייל,התעניינות,סיכום שיחה,מחלקה,מקור,קמפיין,דף נחיתה,הסכמה שיווקית,גרסת הסכמה,זמן הסכמה,סטטוס,אורך שיחה,הקשר מלא,שפה,תאריך,עמוד\n";
    foreach ( $leads as $l ) {
        echo '"' . implode( '","', array_map( fn( $v ) => str_replace( '"', '""', (string) ( $v ?? '' ) ), [
            ...
            $l['lead_status'] ?? '', $l['conversation_length'] ?? '',
