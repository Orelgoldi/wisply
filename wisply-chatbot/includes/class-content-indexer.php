...
class Wisply_Content_Indexer {
    ...
    private function index_menus(): int {
        ...
        foreach ( $menus as $menu ) {
            ...
            $content = "רשימת השירותים, המוצרים והעמודים באתר (מתוך תפריט \"{$menu->name}\"):\n"
                     . implode( "\n", $lines );
    ...
    private function index_footer(): int {
        $response = wp_remote_get( home_url( '/' ), [ 'timeout' => 15, 'user-agent' => 'Wisply-Bot/1.0' ] );
    ...
    private function fetch_rendered_text( int $post_id ): string {
        ...
        if ( ! $url ) return '';
        $response = wp_remote_get( $url, [ 'timeout' => 12, 'user-agent' => 'Wisply-Bot/1.0' ] );
        if ( is_wp_error( $response ) || wp_remote_retrieve_response_code( $response ) !== 200 ) {
            return '';
        }
    ...
    private function collect_sitemap_urls( string $sitemap_url, int $depth = 0 ): ?array {
        ...
        $response = wp_remote_get( $sitemap_url, [ 'timeout' => 15, 'user-agent' => 'Wisply-Bot/1.0' ] );
        if ( is_wp_error( $response ) || wp_remote_retrieve_response_code( $response ) !== 200 ) {
            return null;
        }
