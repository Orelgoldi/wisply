...
if ( isset( $_POST['wisply_reindex_nonce'] ) ) {
    if ( ! check_admin_referer( 'wisply_do_reindex', 'wisply_reindex_nonce' ) ) {
...
// ── Clear-all form POST ─────────────────────────────────────────────────────
if ( isset( $_POST['wisply_clearall_nonce'] ) ) {
    if ( check_admin_referer( 'wisply_clear_all', 'wisply_clearall_nonce' ) && current_user_can( 'manage_options' ) ) {
...
// ── Sitemap form POST ──────────────────────────────────────────────────────
if ( isset( $_POST['wisply_sitemap_nonce'] ) ) {
    if ( check_admin_referer( 'wisply_add_sitemap', 'wisply_sitemap_nonce' ) && current_user_can( 'manage_options' ) ) {
        $url    = esc_url_raw( $_POST['sitemap_url'] ?? '' );
        $result = Wisply_Content_Indexer::get_instance()->index_sitemap( $url );
        if ( $result['success'] ) {
...
if ( isset( $_POST['wisply_url_nonce'] ) ) {
    if ( check_admin_referer( 'wisply_add_url', 'wisply_url_nonce' ) && current_user_can( 'manage_options' ) ) {
        $url    = esc_url_raw( $_POST['single_url'] ?? '' );
        $lang   = sanitize_text_field( $_POST['url_lang'] ?? 'he' );
        $result = Wisply_Content_Indexer::get_instance()->index_url( $url, $lang );
        if ( $result['success'] ) {
...
// ── URL list form POST ─────────────────────────────────────────────────────
if ( isset( $_POST['wisply_urllist_nonce'] ) ) {
    if ( check_admin_referer( 'wisply_add_urllist', 'wisply_urllist_nonce' ) && current_user_can( 'manage_options' ) ) {
        $list   = sanitize_textarea_field( $_POST['url_list'] ?? '' );
        $lang   = sanitize_text_field( $_POST['urllist_lang'] ?? 'he' );
        $result = Wisply_Content_Indexer::get_instance()->index_url_list( $list, $lang );
...
if ( isset( $_POST['wisply_text_nonce'] ) ) {
    if ( check_admin_referer( 'wisply_add_text', 'wisply_text_nonce' ) && current_user_can( 'manage_options' ) ) {
        ...
        $result  = Wisply_Content_Indexer::get_instance()->index_text( $title, $content, $lang );
        if ( $result['success'] ) {
...
$test_question = '';
$test_result   = null;
if ( isset( $_POST['wisply_test_nonce'] ) ) {
    if ( check_admin_referer( 'wisply_test_bot', 'wisply_test_nonce' ) && current_user_can( 'manage_options' ) ) {
        $test_question = sanitize_text_field( $_POST['test_question'] ?? '' );
        $test_lang     = sanitize_text_field( $_POST['test_lang'] ?? 'he' );
        if ( $test_question ) {
            $test_result = Wisply_AI_Handler::get_instance()->diagnose( $test_question, $test_lang );
        }
    }
}
...
$base    = admin_url( 'admin.php?page=wisply-index' );
...
?>
<style>
.wisply-kb { direction:rtl; font-family:"Open Sans Hebrew",Arial,sans-serif; }
.wisply-kb h1 { margin:0; font-size:24px; }
.wisply-source-tabs { display:flex; gap:6px; margin-bottom:16px; flex-wrap:wrap; }
.wisply-source-tab {
...
.wisply-source-tab:hover { border-color:#00A3A3; color:#00A3A3; }
.wisply-source-tab.active { background:#00A3A3; color:#fff; border-color:#00A3A3; }
.wisply-source-panel { display:none; background:#fff; border:1.5px solid #e2e8f0; border-radius:10px; padding:20px; margin-bottom:20px; }
.wisply-source-panel.active { display:block; }
.wisply-source-panel label { font-weight:600; font-size:13px; display:block; margin-bottom:5px; }
.wisply-source-panel input[type=text],
.wisply-source-panel input[type=url],
.wisply-source-panel select,
.wisply-source-panel textarea { width:100%; padding:9px 12px; border:1.5px solid #e2e8f0; border-radius:8px; font-size:14px; direction:rtl; font-family:inherit; margin-bottom:12px; }
.wisply-source-panel textarea { min-height:120px; resize:vertical; }
.wisply-source-panel input:focus, .wisply-source-panel textarea:focus, .wisply-source-panel select:focus {
    border-color:#00A3A3; outline:none; box-shadow:0 0 0 3px rgba(0,163,163,.1);
}
.wisply-btn-primary { background:#00A3A3; color:#fff; border:none; padding:10px 22px; border-radius:8px; font-size:14px; font-weight:700; cursor:pointer; }
.wisply-btn-primary:hover { background:#007878; }
.wisply-btn-primary:disabled { background:#ccc; }
.wisply-stat-bar { display:flex; gap:0; background:#fff; border:1px solid #e5e5e5; border-radius:8px; margin-bottom:20px; overflow:hidden; }
.wisply-stat-item { flex:1; padding:14px 18px; text-align:center; border-left:1px solid #e5e5e5; }
.wisply-stat-item:last-child { border-left:none; }
.wisply-stat-num { font-size:24px; font-weight:700; }
.wisply-stat-lbl { font-size:12px; color:#777; margin-top:2px; }
.wisply-kb-table { width:100%; border-collapse:collapse; background:#fff; border:1px solid #e5e5e5; border-radius:8px; overflow:hidden; }
.wisply-kb-table th { background:#f8fafc; padding:10px 14px; font-size:12px; color:#64748b; font-weight:600; text-align:right; border-bottom:1px solid #e5e5e5; }
.wisply-kb-table td { padding:12px 14px; border-bottom:1px solid #f1f5f9; font-size:13px; vertical-align:middle; }
.wisply-kb-table tr:last-child td { border-bottom:none; }
.wisply-kb-table tr:hover td { background:#f8fafc; }
.wisply-status-ok { background:#dcfce7; color:#15803d; padding:3px 10px; border-radius:12px; font-size:11px; font-weight:700; white-space:nowrap; }
.wisply-type-badge { background:#f1f5f9; padding:2px 8px; border-radius:8px; font-size:11px; font-weight:600; color:#475569; }
.wisply-del { background:none; border:none; cursor:pointer; color:#dc2626; font-size:16px; padding:4px 8px; border-radius:6px; }
.wisply-del:hover { background:#fee2e2; }
.wisply-source-result { margin-top:10px; font-size:13px; font-weight:600; }
</style>

<div class="wrap wisply-kb">
    ...
    <div style="background:#fff;border:1.5px solid #e2e8f0;border-radius:10px;padding:20px;margin-bottom:24px">
        ...
        <div class="wisply-source-tabs">
            <button class="wisply-source-tab active" data-panel="panel-wp">📝 WordPress</button>
            <button class="wisply-source-tab" data-panel="panel-sitemap">🗺️ Sitemap</button>
            <button class="wisply-source-tab" data-panel="panel-url">🔗 URL בודד</button>
            <button class="wisply-source-tab" data-panel="panel-url-list">📋 רשימת URLs</button>
            <button class="wisply-source-tab" data-panel="panel-text">✏️ טקסט ידני</button>
        </div>

        <!-- WordPress panel — standard form POST, no AJAX -->
        <div id="panel-wp" class="wisply-source-panel active">
            ...
            <form method="post" action="" style="display:inline">
                <?php wp_nonce_field( 'wisply_do_reindex', 'wisply_reindex_nonce' ); ?>
                <button type="submit" class="wisply-btn-primary">↺ אנדקס תוכן WordPress עכשיו</button>
            </form>
        </div>

        <!-- Sitemap panel -->
        <div id="panel-sitemap" class="wisply-source-panel">
            ...
            <form method="post" action="">
                <?php wp_nonce_field('wisply_add_sitemap','wisply_sitemap_nonce'); ?>
                <label>כתובת Sitemap</label>
                <input type="url" name="sitemap_url" value="<?php echo esc_attr(home_url('/sitemap.xml')); ?>" required dir="ltr">
                <button type="submit" class="wisply-btn-primary" style="margin-top:4px">אנדקס Sitemap</button>
            </form>
        </div>

        <!-- Single URL panel -->
        <div id="panel-url" class="wisply-source-panel">
            <form method="post" action="">
                <?php wp_nonce_field('wisply_add_url','wisply_url_nonce'); ?>
                <label>כתובת URL</label>
                <input type="url" name="single_url" placeholder="https://wisply.org/about" required dir="ltr">
                ...
                <button type="submit" class="wisply-btn-primary" style="margin-top:4px">אנדקס URL</button>
            </form>
        </div>

        <!-- URL list panel -->
        <div id="panel-url-list" class="wisply-source-panel">
            <form method="post" action="">
                <?php wp_nonce_field('wisply_add_urllist','wisply_urllist_nonce'); ?>
                <label>רשימת URLs (אחד בשורה)</label>
                <textarea name="url_list" placeholder="https://wisply.org/page1&#10;https://wisply.org/page2" dir="ltr"></textarea>
                <label style="margin-top:4px">שפה</label>
                ...
                <button type="submit" class="wisply-btn-primary" style="margin-top:4px">אנדקס רשימה</button>
            </form>
        </div>

        <!-- Manual text panel -->
        <div id="panel-text" class="wisply-source-panel">
            <form method="post" action="">
                <?php wp_nonce_field('wisply_add_text','wisply_text_nonce'); ?>
                <label>כותרת</label>
                ...
                <button type="submit" class="wisply-btn-primary" style="margin-top:4px">הוסף טקסט</button>
            </form>
        </div>
    </div>
    ...
    <div style="background:#fff;border:1.5px solid #e2e8f0;border-radius:10px;padding:20px;margin-bottom:24px">
        ...
        <form method="post" action="">
            <?php wp_nonce_field( 'wisply_test_bot', 'wisply_test_nonce' ); ?>
            <div style="display:flex;gap:8px;align-items:flex-end;flex-wrap:wrap">
                <input type="text" name="test_question" value="<?php echo esc_attr( $test_question ); ?>"
                       placeholder="למשל: מהן שעות הפעילות שלכם?" required
                ...
                <button type="submit" class="wisply-btn-primary">בדוק</button>
            </div>
        </form>
    ...
    <!-- Stats bar -->
    <div class="wisply-stat-bar">
        <div class="wisply-stat-item">
            <div class="wisply-stat-num"><?php echo (int) $total; ?></div>
            <div class="wisply-stat-lbl">מקורות</div>
        </div>
        <div class="wisply-stat-item">
            <div class="wisply-stat-num" style="color:#22c55e"><?php echo (int) $total; ?></div>
            <div class="wisply-stat-lbl">● מעובדים</div>
        </div>
        <div class="wisply-stat-item">
            <div class="wisply-stat-num" style="color:#f59e0b">0</div>
            <div class="wisply-stat-lbl">● ממתינים</div>
        </div>
    </div>
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:12px;flex-wrap:wrap">
        <form method="get" style="margin:0">
            <input type="hidden" name="page" value="wisply-index">
        ...
        <form method="post" style="margin:0"
            ...
            <?php wp_nonce_field( 'wisply_clear_all', 'wisply_clearall_nonce' ); ?>
    ...
    <table class="wisply-kb-table">
        ...
        <?php foreach ($rows as $row) : ?>
            <tr id="wisply-row-<?php echo (int)$row['post_id']; ?>-<?php echo esc_attr($row['lang']); ?>">
                <td>
                ...
                <td style="text-align:center"><span class="wisply-type-badge"><?php echo esc_html(strtoupper($row['lang'])); ?></span></td>
                <td style="text-align:center">
                    <span class="wisply-type-badge">
                        <?php
                        $type_labels = [
                ...
                <td style="text-align:center"><span class="wisply-status-ok">✓ Processed</span></td>
                <td style="text-align:center;color:#777;font-size:12px">
                    <?php echo esc_html(date_i18n('j M Y', strtotime($row['indexed_at']))); ?>
                </td>
                <td style="text-align:center">
                    <button class="wisply-del wisply-del-btn"
...
<script>
(function() {
    const nonce   = (window.WisplyAdmin || {}).ajaxNonce || '';
    const ajax    = window.ajaxurl || '/wp-admin/admin-ajax.php';

    // ── Tab switching ─────────────────────────────────────────────────
    document.querySelectorAll('.wisply-source-tab').forEach(tab => {
        tab.addEventListener('click', function() {
            document.querySelectorAll('.wisply-source-tab').forEach(t => t.classList.remove('active'));
            document.querySelectorAll('.wisply-source-panel').forEach(p => p.classList.remove('active'));
    ...
    document.querySelectorAll('.wisply-add-source-btn').forEach(btn => {
        btn.addEventListener('click', async function() {
            const type   = this.dataset.type;
            const result = document.getElementById('result-' + type.replace('_','-'));
            const fd     = new FormData();
            fd.append('action', 'wisply_add_source');
    ...
    // ── Delete item ───────────────────────────────────────────────────
    document.querySelectorAll('.wisply-del-btn').forEach(btn => {
        btn.addEventListener('click', async function() {
            if (!confirm('הסר פריט זה מהאינדקס?')) return;
            this.disabled = true;
            const fd = new FormData();
            fd.append('action','wisply_delete_indexed'); fd.append('nonce',nonce);
            fd.append('post_id', this.dataset.postId); fd.append('lang', this.dataset.lang);
            const res  = await fetch(ajax, {method:'POST', body:fd});
            const json = await res.json();
            if (json.success) {
                document.getElementById('wisply-row-'+this.dataset.postId+'-'+this.dataset.lang)?.remove();
            } else {
                this.disabled = false;
                alert('שגיאה במחיקה');
            }
